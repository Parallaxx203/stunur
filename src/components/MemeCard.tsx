...
export function MemeCard({ meme }: { meme: Meme }) {
  ...
  // Load initial stats + liked state
  React.useEffect(() => {
    let mounted = true;
  ...
  // View tracking via IntersectionObserver (one count per session per card)
  React.useEffect(() => {
    ...
    >
      <div className="flex-1 bg-zinc-900 relative overflow-hidden">
        ...
        <button
          ...
          className={cn(
            ...
            liked
              ? 'bg-red-600/30 border-red-500/60 text-red-400 shadow-[0_0_20px_rgba(255,0,0,0.4)]'
      ...
      <div className="p-4 border-t border-white/10 bg-[#050505] flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3 text-[9px] font-pixel text-white/50">
          ...
          <span className="flex items-center gap-1">
            <Heart className={cn('w-3 h-3', liked && 'fill-red-500 text-red-500')} />
            {likes}
          </span>
        </div>
      </div>
